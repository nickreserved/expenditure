...
function chk_order($a, $warning = true) {
	if (preg_match('/Φ\.?\d{3}(\.\d+)?\/\d+\/\d+\/Σ\.?\d+\/\d{2} (Ιαν|Φεβ|Μαρ|Απρ|Μαι|Ιουν|Ιουλ|Αυγ|Σεπ|Οκτ|Νοε|Δεκ) \d{4}\/.+/', $a))
		return chk($a);
...
function chk_date($a) {
	if (preg_match('/\d{1,2} (Ιαν|Φεβ|Μαρ|Απρ|Μαι|Ιουν|Ιουλ|Αυγ|Σεπ|Οκτ|Νοε|Δεκ) \d{4}/', $a))
		return chk($a);
	trigger_error(($a ? "Το '<b>$a</b>' δεν είναι ημερομηνία" : 'Οι ημερομηνίες πρέπει να δίνονται') . " στη μορφή π.χ. '20 Νοε 2005'");
}
...
function get_order($a) {
	$m = null;
	if (preg_match('/(Φ\.?\d{3}(\.\d+)?)\/(\d+)\/(\d+)\/(Σ\.?\d+)\/(\d{2} (Ιαν|Φεβ|Μαρ|Απρ|Μαι|Ιουν|Ιουλ|Αυγ|Σεπ|Οκτ|Νοε|Δεκ) \d{4})\/(.+)/', $a, $m))
		return array('Φάκελος' => $m[1], 'Υποφάκελος' => $m[3], 'Πρωτόκολλο' => $m[4],
			'Σχέδιο' => $m[5], 'Ημερομηνία' => $m[6], 'Εκδότης' => $m[8]);
	chk_order($a);
}
...
function getBrigate($a) {
	...
	if (preg_match('/([^\/]+)\/.+/', $a, $m))
		return $m[1];
	trigger_error("Το '<b>$a</b>' δεν είναι σωστή διεύθυνση Γραφείου Σχηματισμού");
}
