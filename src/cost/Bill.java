...
public class Bill extends DynHashObject {
	...
	protected final void recalculate() {
		...
		try {
			...
			getDynamic().put("ΚαθαρήΑξία", ka = Functions.round(d, 2));
			...
			double sum = Functions.round(ka * (double) hold.getDynamic().get("Σύνολο") / 100, 2);
			...
			getDynamic().put("Καταλογιστέο", kat = Functions.round(kat + ka, 2));

			getDynamic().put("Πληρωτέο", pl = Functions.round(kat - thold, 2));
			...
			getDynamic().put("ΦΕΣεΕυρώ", fee = Functions.round(kak * fe / 100.0, 2));

			getDynamic().put("ΥπόλοιποΠληρωτέο", Functions.round(pl - fee, 2));
	...
	protected void setFpa() {
		...
		for (BillItem bi : items)
			if ("Στρατός".equals(a) || "Ενοικιαστής".equals(a)) bi.put("ΦΠΑ", (byte) 0);
	}
}