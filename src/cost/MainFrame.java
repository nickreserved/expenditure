...
final public class MainFrame extends JFrame implements ActionListener {
	...
	static private void openCost(String file) {
		try {
			...
			showExceptionMessage(ths, e, "Άνοιγμα αρχείου",
	...
	static private void importCostContents(Object o, int flags) throws Exception {
		...
		if (o instanceof Cost) {
			...
			if ((flags & 2) != 0) b.stream()
					.map(i -> (Hold) i.get("ΑνάλυσηΚρατήσεωνΣεΠοσοστά"))
					.filter(i -> i != null && !holds.contains(i))
					.forEach(i -> holds.add(i));
			if ((flags & 4) != 0) b.stream()
					.map(i -> (Provider) i.get("Προμηθευτής"))
			...
			if ((flags & 1) != 0)
				((VectorObject<Man>) ((HashObject) o).get("Προσωπικό")).stream()
						.filter(i -> !men.contains(i))
						.forEach(i -> men.add(i));
			if ((flags & 2) != 0)
				((VectorObject<Hold>) ((HashObject) o).get("Κρατήσεις")).stream()
						.filter(i -> !holds.contains(i))
						.forEach(i -> holds.add(i));
			if ((flags & 4) != 0)
				((VectorObject<Provider>) ((HashObject) o).get("Προμηθευτές")).stream()
	...
	public static void main(String[] args) {
		...
		// Αν έχουμε νέα έκδοση στο πρόγραμμα, προστίθενται οι νέες κρατήσεις από το ενσωματωμένο αρχείο ρυθμίσεων
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (((JMenu) getMenuFromName("Κέλυφος ")).isMenuComponent(j)) {
			...
			if (a && c != null) {
				((ArrayList<Bill>) c.get("Τιμολόγια")).forEach(i -> i.recalculate());
				repaint();
			}
		}
