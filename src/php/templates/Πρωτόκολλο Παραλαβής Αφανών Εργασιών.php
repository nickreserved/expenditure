...
foreach($data['Εργασίες'] as $v) {
	?>\qr <?=++$count?>\cell\qj <?=chk($v['Εργασία'])?>\cell\qc <?=chk_measure($v['ΜονάδαMέτρησης'])?>\cell <?=num($v['Ποσότητα'])?>\cell\cell\row<?
} ?>
\pard\plain\qr\par <?=chk($data['Πόλη']) . ', ' . $data['ΗμερομηνίαΑφανώνΕργασιών']?>\par
