...
\fs24\ul\qc ΠΡΩΤΟΚΟΛΛΟ\line ΕΚΤΕΛΕΣΘΕΝΤΩΝ ΕΡΓΑΣΙΩΝ\par\par
...
που συγκροτήθηκε με την \ul <?=chk_order($data['ΔγηΑνάθεσης'])?>\ul0 , βεβαιώνει οτι για την πραγματοποίηση των εργασιών, αναλώθηκαν υλικά όπως παρακάτω:\par
<?
$count = 0;
foreach($data['Εργασίες'] as $v) {
	echo '\par\tab ' . ++$count . '.\tab\b\ul ' . chk($v['Εργασία']) . '\ul0\b0  (' . num($v['Ποσότητα']) . ' ' . chk_measure($v['ΜονάδαMέτρησης']) . ')\par';
	$count1 = 0;
