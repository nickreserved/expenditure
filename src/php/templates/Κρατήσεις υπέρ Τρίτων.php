...
?>
 Κρατήσεις<? if (count($bills_hold_all) == 1) echo '\line ' . percent($bills[0]['ΑνάλυσηΚρατήσεωνΣεΠοσοστά']['Σύνολο'])?>\cell Υπόλοιπο Πληρωτέο\b0\cell\row
...
foreach($bills_hold_all as $v) {
	...
	foreach($v as $i) {
		...
		echo euro($i['ΑνάλυσηΚρατήσεωνΣεΕυρώ']['Σύνολο']) . '\cell ' . euro($i['ΥπόλοιποΠληρωτέο']) . '\cell\row';
	}
} ?>
...
<?=euro($bills_info['ΥπόλοιποΠληρωτέο'])?>\cell\row
