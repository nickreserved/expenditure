...
foreach($data['Τιμολόγια ανά Δικαιούχο'] as $per_contractor) {
	...
	foreach(get_invoices_by_category($invoices) as $category => $invoices) {
		...
		if ($category == 'Προμήθεια Υλικών')
			...
			$c = $per_contractor['Δικαιούχος']['Τύπος'] == 'Ιδιωτικός Τομέας'
...
\tab\tab α.\tab Τους όρους της <?=contract($per_contractor['Σύμβαση']['Σύμβαση'])?> σύμβασης.\par
...
- Ο -\line <?=$a[7]?>\line\line\line <?=rtf($per_contractor['Δικαιούχος']['Επωνυμία'])?>\cell\row
