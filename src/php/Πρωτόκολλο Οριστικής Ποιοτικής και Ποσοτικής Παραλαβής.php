	...
	foreach($data['Συμβάσεις'] as $per_contract) {
		...
		foreach(get_invoices_by_category($invoices) as $category => $invoices) {
			...
			if ($category == 'Προμήθεια Υλικών')
				...
				$c = $per_contract['Δικαιούχος']['Τύπος'] == 'Ιδιωτικός Τομέας'
...
\tab 1.\tab Σήμερα την <?=strftime('%d %b %y', get_newer_invoice_timestamp($invoices))?> συνήλθε η Επιτροπή Παρακολούθησης και Παραλαβής <?=$a[1]?>, που συγκροτήθηκε με την <?=order($data['Δγη Συγκρότησης Επιτροπών'])?> για να προβεί στην παραλαβή των <?=$a[0]?> που παραδόθηκαν από <?=$a[3]?> «<?=$per_contract['Δικαιούχος']['Επωνυμία']?>». Στη συνεδρίαση παρόντες ήταν οι:\par
...
\tab\tab α.\tab Τους όρους της <?=contract($per_contract['Σύμβαση']['Σύμβαση'])?> σύμβασης.\par
...
- Ο -\line <?=$a[7]?>\line\line\line <?=rtf($per_contract['Δικαιούχος']['Επωνυμία'])?>\cell\row
...
unset($a, $c, $category, $invoices, $per_contract);
