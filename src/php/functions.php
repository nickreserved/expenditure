...
function rtf_close($file) {
	...
	if ($file == $a[0]) echo PHP_EOL . PHP_EOL . PHP_EOL . '}';
}
 ...
 * 'Τχη (ΜΧ) Γκέσου Παύλου (AM: 12345)', με τo AM να εξάγεται μόνο αν υπάρχει */
function personi($a, $c) {
	return inflectPhrase(person($a), $c) . (isset($a['ΑΜ']) ? " (ΑΜ: {$a['ΑΜ']})" : null);
}
...
function has_direct_assignment() {
	...
	if (isset($data['Συμβάσεις']))
		foreach($data['Συμβάσεις'] as $per_contract)
			if (!isset($per_contract['Διαγωνισμός'])) return true;
	return false;
}
...
function explode_order(& $order) {
	$a = null;
	if (preg_match('/^(Φ\.?\d{3}(\.\d+)?)\/(\d+)\/(\d+)\/(Σ\.?\d+)\/(\d{1,2} (Ιαν|Φεβ|Μαρ|Μάρ|Απρ|Μαι|Μάι|Ιουν|Ιούν|Ιουλ|Ιούλ|Αυγ|Αύγ|Σεπ|Οκτ|Νοε|Νοέ|Δεκ) (\d{2}))\/([^\\\{\}\:]+)( \((ΑΔΑ)\: ?([Α-Ω0-9]{5}-[Α-Ω0-9]{3})(?:, ΑΔΑΜ\: ?([Α-ΩA-Z0-9]{14}))?\)| \((ΑΔΑΜ)\: ?([Α-ΩA-Z0-9]{14})(?:, ΑΔΑ\: ?([Α-Ω0-9]{5}-[Α-Ω0-9]{3}))?\))?$/', $order, $a)) {
		$order = array(
			'Ταυτότητα' => $a[0],
		...
		if (isset($a[11]))
			if ($a[11] == 'ΑΔΑ') {
				$order['ΑΔΑ'] = $a[12];
				if (isset($a[13])) $order['ΑΔΑΜ'] = $a[13];
			} else { // 'ΑΔΑΜ'
				$order['ΑΔΑΜ'] = $a[12];
				if (isset($a[13])) $order['ΑΔΑ'] = $a[13];
			}
	} else
...
function init_contracts() {
	...
	foreach($data['Συμβάσεις'] as & $contract) {
		explode_order_if_exists($contract, 'Πρόσκληση Υποβολής Προσφορών');
		explode_order_if_exists($contract, 'Απόφαση Απευθείας Ανάθεσης');
 ...
 * <p>Πρέπει να κληθεί μετά από την init_contracts().
 * <p>Επίπεδο αρχικοποίησης 5. */
function init_invoices() {
	...
	foreach($data['Τιμολόγια'] as & $invoice) {
		...
		// Υπολογίζει καθαρή αξία και ΦΠΑ τιμολογίου
		$net = $vat = 0;
		$vat_items = array();
		...
		// Επανέρχεται στα είδη για να βάλει τις αξίες ΦΠΑ κατά είδος
		$vat_items = adjust_partials($vat_items, $vat);
		foreach($invoice['Είδη'] as $i => & $item) {
			$item['Τιμή ΦΠΑ'] = $vat_items[$i];
			$item['Συνολική Τιμή με ΦΠΑ'] = $item['Συνολική Τιμή'] + $vat_items[$i];
		}
		unset($item);
		// Υπολογισμός επιμέρους κρατήσεων
		$a = array();
		$deduction = $invoice['Κρατήσεις'];
		$ratio = $deductions / $deduction['Σύνολο'];
		foreach($deduction as $name => $term)
			if ($name != 'Σύνολο')
				$a[$name] = $ratio * $term;
		// Σφάλματα στρογγυλοποίησης
		$invoice['Ανάλυση Κρατήσεων'] = adjust_partials($a, $deductions);
		// Τοποθέτηση στο τιμολόγιο των τιμών
		$invoice['Τιμές'] = array(
			'Καθαρή Αξία'        => $net,
			'ΦΠΑ'                => $vat,
			'Καταλογιστέο'       => $mixed,
 ...
 * <li>Με κλειδί 'Κατηγορία' το κυρίαρχο είδος των τιμολογίων (έργο, προμήθεια ή υπηρεσία).
 ...
 * <li>Με κλειδί 'Διαγωνισμός' το διαγωνισμό.
 * <li>Με κλειδί 'Χρόνος Υπογραφής Συμβάσεων' η ημερομηνία της νεότερης σύμβασης.
 * <li>Με κλειδί 'Κατηγορία' το κυρίαρχο είδος των τιμολογίων (έργο, προμήθεια ή υπηρεσία).</ul>
