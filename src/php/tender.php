...
\line - Ο -\line ΠΡΟΕΔΡΟΣ\line\line\line <?=rtf($data['Πρόεδρος Διαγωνισμών']['Ονοματεπώνυμο'])?>\line <?=rtf($data['Πρόεδρος Διαγωνισμών']['Βαθμός'])?>\cell
\line - ΤΑ -\line ΜΕΛΗ\line\line\line <?=rtf($data['Α Μέλος Διαγωνισμών']['Ονοματεπώνυμο'])?>\line <?=rtf($data['Α Μέλος Διαγωνισμών']['Βαθμός'])?>
\line\line\line <?=rtf($data['Β Μέλος Διαγωνισμών']['Ονοματεπώνυμο'])?>\line <?=rtf($data['Β Μέλος Διαγωνισμών']['Βαθμός'])?>\cell
ΘΕΩΡΗΘΗΚΕ\line - Ο -\line ΔΚΤΗΣ\line\line\line <?=rtf($data['Δκτης']['Ονοματεπώνυμο'])?>\line <?=rtf($data['Δκτης']['Βαθμός'])?>\cell\row
...
\tab β.\tab <?=personi($data['Α Μέλος Διαγωνισμών'], 2)?> και\par
\tab γ.\tab <?=personi($data['Β Μέλος Διαγωνισμών'], 2)?> ως μέλη,\par
βάσει της διοικητικής πράξης <?=$data['Δγη Συγκρότησης Επιτροπών']['Ταυτότητα']?>, συνήλθε σε ολομέλεια, για την αποσφράγιση των δικαιολογητικών - τεχνικών προσφορών της ανοικτής διαδικασίας που προκυρήχθηκε με την <?=$tender['Προκύρηξη Διαγωνισμού']['Ταυτότητα']?>, με συστημικό αριθμό ΕΣΗΔΗΣ <?='____________'?>, για <?=$tender['τίτλος']?> και αποφαίνεται ως ακολούθως:\par
\tab α.\tab Ο διαγωνισμός και η αποσφράγιση των δικαιολογητικών συμμετοχής - τεχνικών προσφορών, έγινε δημόσια, σύμφωνα με τον ν. 4412/2016, τηρουμένων των χρονοδιαγραμμάτων και των διαδικασιών οι οποίες ορίζονται.\par
\tab β.\tab Κατά τη συνεδρίαση, έλαβε χώρα η ηλεκτρονική αποσφράγιση, μέσω σχετικής λειτουργίας του ΕΣΔΗΣ, των φακέλων «Δικαιολογητικά Συμμετοχής - Τεχνική Προσφορά», των παρακάτω οικονομικών φορέων:\par
<?php
	foreach($tender['Ενδιαφερόμενοι'] as $count => $competitor) {
		echo '\tab\tab (' . $count + 1 . ')\tab ' . rtf(get_contractor_id($competitor['Δικαιούχος'])) . ', με δικαιολογητικά συμμετοχής:\par' . PHP_EOL;
		foreach($competitor['Δικαιολογητικά Συμμετοχής'] as $count2 => $i)
			echo '\tab\tab\tab (' . greeknum($count2 + 1) . ')\tab ' . rtf($i) . '.\par' . PHP_EOL;
	}
?>
2.\tab Ακολούθως η επιτροπή εξακρίβωσε την πληρότητα των στοιχείων των δικαιολογητικών συμμετοχής και αξιολόγησε τις τεχνικές προσφορές, διαπιστώνοντας <?php
	$rejected = $tender['Απορριφθέντες Α'];
	$caccepted = count($tender['Προκριθέντες Α']);
	// Κείμενο σχετικό με το ποιος απορρίφθηκε.
	if (count($rejected)) {
		if (!$caccepted)
			echo 'ότι κανένας οικονομικός φορέας δεν πληρεί τις απαιτήσεις της διακήρυξης, για τους παρακάτω λόγους:\par' . PHP_EOL;
		else echo 'ότι οι παρακάτω οικονομικοί φορείς δεν πληρούν τις απαιτήσεις της διακήρυξης, για τους λόγους που αναφέρονται:\par' . PHP_EOL;
		foreach($rejected as $count => $competitor) {
			echo '\tab ' . greeknum($count + 1) . '\tab ' . rtf(get_contractor_id($competitor['Δικαιούχος'])) . '.\par' . PHP_EOL;
			foreach($competitor['Λόγοι Απόρριψης Συμμετοχής'] as $count2 => $i)
				echo '\tab\tab (' . $count2 + 1 . ')\tab ' . rtf($i) . '.\par' . PHP_EOL;
		}
		if (!$caccepted)
			echo '3.\tab Λαμβάνοντας υπόψη τα παραπάνω, η επιτροπή προτείνει τη διακοπή της ηλεκτρονικής ανοικτής διαδικασίας, καθώς κανένας οικονομικός φορέας δεν συμμορφώνεται με τις απαιτήσεις της διακήρυξης.\par' . PHP_EOL;
	} else echo 'ότι όλοι οι οικονομικοί φορείς πληρούν τις απαιτήσεις της διακήρυξης.\par' . PHP_EOL;
	if ($caccepted) {
		echo '3.\tab Λαμβάνοντας υπόψη τα παραπάνω, η επιτροπή προτείνει τη συνέχιση της ηλεκτρονικής ανοικτής διαδικασίας, με τους οικονομικούς φορείς:\par' . PHP_EOL;
		foreach($tender['Προκριθέντες Α'] as $count => $competitor)
			echo '\tab ' . greeknum($count + 1) . '\tab ' . rtf(get_contractor_id($competitor['Δικαιούχος'])) . '.\par' . PHP_EOL;
		echo 'και την ηλεκτρονική αποσφράγιση των οικονομικών προσφορών σε ημερομηνία που θα ορίσει ' . rtf(article(gender($data['Μονάδα Πλήρες']), 0) . ' ' . inflectPhrase($data['Μονάδα Πλήρες'], 0)) . '\par' . PHP_EOL;
	}
?>
4.\tab το παρόν πρακτικό κλείνεται και υπογράφεται όπως ακολουθεί:\par
<?php
	tender_committee_signs();
}
